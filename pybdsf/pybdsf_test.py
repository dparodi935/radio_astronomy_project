...
img = bdsf.process_image(os.path.join(fits_path,f'{fits_file}.fits'), thresh_pix=5.0, thresh_isl=4.0, adaptive_rms_box=True, ncores=2)

#adaptive rms box means the box used to measure noise shrinks near bright sources to capture the noise artefacts
img_atrous = bdsf.process_image(os.path.join(fits_path,f'{fits_file}.fits'), thresh_pix=5.0, thresh_isl=4.0, adaptive_rms_box=True, ncores=2, atrous_do=True)
...
img_atrous.write_catalog(outfile=os.path.join(fits_path, f'{fits_file}_sources_atrous.fits'), format='fits', clobber=True)